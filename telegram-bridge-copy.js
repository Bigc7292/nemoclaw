...
function runAgentInSandbox(message, sessionId) {
  return new Promise((resolve) => {
    ...
    const cmd = `export NVIDIA_API_KEY='${API_KEY}' && openclaw agent --agent main --local -m '${escaped}' --session-id 'tg-${sessionId}'`;
...
async function processUpdate(update) {
  ...
  // Handle /debug
  if (msg.text === "/debug") {
    await sendMessage(chatId, `Mode: Polling\nProcessed Updates (cache): ${processedUpdates.size}`, msg.message_id);
    return;
  }
  ...
  try {
    ...
    if (res.ok && res.result?.length > 0) {
      for (const update of res.result) {
        ...
        // Fire and forget
        processUpdate(update).catch(err => console.error("processUpdate error:", err));
      }
    }
  } catch (err) {
